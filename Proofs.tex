...
The second nontrivial metric is \(\text{inertvars}(\inertset, \tau)\): the number of \emph{distinct} variables mentioned anywhere (i.e. in \(W\) or in the image of \(\inertset\) over \(W\)) which are mapped by \(\inertset\) to themselves.\footnote{Make sure to contrast \(\text{inertvars}(\inertset, \tau)\) with \(\text{potentoccs}(\inertset, \tau)\); the former counts distinct variables reflexively mapped; the latter counts variable name occurrences irreflexively mapped.}
