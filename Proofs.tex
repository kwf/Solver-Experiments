...
\usepackage{amsfonts, amsmath, amssymb, amsthm, enumitem, graphicx, hanging, ifthen, lettrine, listings, microtype, multirow, mathtools, natbib, pdfpages, rotating, scalefnt, setspace, textcomp, thinsp, verbatim, xcolor, xspace}
...
\newcommand{\admit}{{\color{red}\bf admit}}
...
% I've been able to phrase everything in terms of treating the inert set as a mere function -- no concrete operations needed. That's neat.

% Size only increases when we rewrite with inert set, which decreases irrefloccs occurrences
...
% THE KEY: Kicking out is equivalent to kicking out after rewriting with new inert set! This means that in kicking-out circumstances, we decrease the irrefloccs metric, since we trade an (a |-> t) for some (r ~ s) where neither r nor s mention a. The actual algorithm defers this rewriting until the kicked-out item is reprocessed, but these two orderings are equivalent (just, one is easier to reason about).

% WAIT A SECOND. If this is true, then does kicking out (at least, K1 kicking-out) do *anything*? THE FOLLOWING IS FALSE: "We kick out {a ~ I(a)}, but as soon as this is re-drawn from the work set, we turn it into I*(a) ~ I*(I(a)), which is I*(a) ~ I*(a), which will be immediately discarded, as it's just refl."

% ^ The above is wrong. Why? We do kick out (a ~ I(a)), but when we rewrite it, we rewrite it with a *new* I, and in particular a new I that contains a new rewrite for a: t. So this is *actually* equivalent to kicking out the equation t ~ I(a), which is equivalent to kicking out I*(t) ~ I*(a).

% Note that this reasoning means that the "weird" kick out rule that I have not included in this document shouldn't prevent termination (??), but is not necessary. (Is it? Hm.)

% S is M.G.U.: forall S', S'(t1) ~ S'(t2), exists S'', S <> S'' = S'
